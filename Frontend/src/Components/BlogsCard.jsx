        ...
        <div className="flex flex-col items-center mt-[100px]">
          ...
          <div className="grid mt-10 ml-10 mr-4 grid-cols-2 gap-8 min-w-[400px] sm:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4">
            {filteredBlogs.map((Blog, index) => (
              <div
                key={Blog.id}
                className="border-cyan-600 hover:shadow-2xl text-[20px] text-pretty text-2xl bg-blue-50 border-4 rounded-md p-6"
              >
                <div className="text-2xl text-purple-950">
                  <span className="font-bold">S.No :</span> {index + 1}
                </div>
                <div className="mt-6">
                  <span className="font-bold ">Topic :</span> {Blog.topic}
                </div>
                <div className="mt-4">
                  <span className="font-bold ">Status :</span> {Blog.status}
                </div>
                <div className="mt-4">
                ...
                <div className="flex items-center justify-evenly mt-6 mr-2 text-4xl">
                  ...
                  <CiCalendarDate
                    className="text-yellow-500 text-5xl cursor-pointer"
                    onClick={() => openDatesModal(index)}
                  />
                </div>
                {blogsModal && selectedIndex === index && (
                  <BlogsModal onClose={closeBlogsModal} Blog={Blog} />
                )}
                {datesModal && selectedIndex === index && (
                  <DatesModal onClose={closeDatesModal} Blog={Blog} />
                )}
              </div>
            ))}
          </div>
